...
//Rare exceptions to indicate a bug in the _customerrecovery code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_customerrecovery = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_customerrecovery]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.customerrecovery.module"
