...
NSString * CustomerRecovery$ModuleRequireFormat = @"(function(exports){"
...
@implementation CustomerRecoveryObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_customerrecovery);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create CustomerRecovery global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "CustomerRecovery" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_customerrecovery = [[CustomerRecoveryObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_customerrecovery];
	
	NSString *_customerrecoveryNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _customerrecoveryNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_customerrecovery valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:CustomerRecovery$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.customerrecovery.kroll" 
	...
	@throw [NSException exceptionWithName:@"org.customerrecovery.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
