...
NSString * StroutFred_InfoApp$ModuleRequireFormat = @"(function(exports){"
...
@implementation StroutFred_InfoAppObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_stroutfred_infoapp);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create StroutFred_InfoApp global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "StroutFred_InfoApp" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_stroutfred_infoapp = [[StroutFred_InfoAppObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_stroutfred_infoapp];
	
	NSString *_stroutfred_infoappNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _stroutfred_infoappNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_stroutfred_infoapp valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:StroutFred_InfoApp$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.stroutfred_infoapp.kroll" 
	...
	@throw [NSException exceptionWithName:@"org.stroutfred_infoapp.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
